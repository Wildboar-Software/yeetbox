...
/// An authorizer that allows read-only authentication for any users, included
/// anonymous users, and allows any write operations for authenticated users.
...
#[tonic::async_trait]
impl Authorizer for SimpleAuthz {
    ...
    async fn is_authz_watch_many(
        ...
        Ok(session.auth_mech != "ANONYMOUS")
    }
